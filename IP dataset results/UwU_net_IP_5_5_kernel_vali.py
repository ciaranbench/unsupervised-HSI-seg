...
whole_image = np.copy(train_set)
vali_set = train_set[12000:]
x_val = vali_set
train_set = train_set[0:12000]
...
BATCH_SIZE = 80
UPDATE_NUM = 10
THRESHOLD = (12000/BATCH_SIZE) * UPDATE_NUM
EPOCHS = 500
...
input_img = keras.Input(shape=(10, 10, 200))
...
u1_up_a = layers.Dense(125, activation='relu')(u1_encoded)
u1_up_b = layers.Reshape((5,5,5))(u1_up_a)
...
u2_up_a = layers.Dense(125, activation='relu')(u2_encoded)
u2_up_b = layers.Reshape((5,5,5))(u2_up_a)
...
u3_up_a = layers.Dense(125, activation='relu')(u3_encoded)
u3_up_b = layers.Reshape((5,5,5))(u3_up_a)
...
u4_up_a = layers.Dense(125, activation='relu')(u4_encoded)
u4_up_b = layers.Reshape((5,5,5))(u4_up_a)
...
u5_up_a = layers.Dense(125, activation='relu')(u5_encoded)
u5_up_b = layers.Reshape((5,5,5))(u5_up_a)
...
decoded = layers.Conv2D(200, (5, 5), activation='relu', padding='same')(x)
...
class ETE(object):
 ...
 def fit(self, x, y=None, batch_size=BATCH_SIZE, maxiter=2e5, tol=1e-3, cae_weights=None, save_dir='./', validation_data = x_val):
     ...
     encoded_imgs_pretrain = self.encoder.predict(whole_image)
...
# Execute training
ete = ETE(input_shape=(10, 10, 200), n_clusters=CLUSTER_NUM)
ete.model.summary()
optimizer = 'adam'
...
ete.fit(train_set, y=train_set, tol=1e-3, maxiter=2e5, save_dir='./',
    cae_weights='model_499.hd5',validation_data=vali_set)

# Acquire results on the training set from trained network
cluster_res = ete.predict(whole_image)

...
vali_set = np.expand_dims(vali_set,axis=4)
...
input_img = keras.Input(shape=(10, 10, 800, 1))
x = layers.Conv3D(10, (10, 10, 10), activation='relu', padding='same')(input_img)
#x = layers.MaxPooling2D((2, 2), padding='same')(x)
x = layers.Conv3D(10, (10, 10, 10), activation='relu', padding='same')(x)
u1_a = layers.MaxPooling3D((2, 2, 2), padding='same')(x)
u1_b = layers.Conv3D(1, (5, 5, 5), activation='relu', padding='same')(u1_a)
u1_c = layers.Flatten()(u1_b)
u1_encoded = layers.Dense(100, activation='relu', name= 'embedding')(u1_c)
u1_up_a = layers.Dense(10000, activation='relu')(u1_encoded)
u1_up_b = layers.Reshape((5,5,400,1))(u1_up_a)
u1_up_c = layers.Conv3D(10, (5, 5, 5), activation='relu', padding='same')(u1_up_b)
u1_up_d = layers.Conv3D(10, (5, 5, 5), activation='relu', padding='same')(u1_up_c)
u1_up_e = layers.UpSampling3D((2, 2, 2))(u1_up_d)
u1_up_f = layers.Conv3D(10, (5, 5, 5), activation='relu', padding='same')(u1_up_e)
u1_up_g = layers.Conv3D(10, (5, 5, 5), activation='relu', padding='same')(u1_up_f)
...
decoded = layers.Conv3D(1, (5, 5, 5), activation='relu', padding='same')(u1_up_g)
...
class ETE(object):
 ...
 def pretrain(self, x, batch_size=BATCH_SIZE, epochs=EPOCHS, optimizer='adam', save_dir='./', validation_data = x_val):
     ...
     # begin training
     self.cae.fit(x, x, batch_size=batch_size, epochs=epochs, callbacks=[csv_logger],validation_data=(x_val, x_val))
...
# Execute training
ete = ETE(input_shape=(10, 10, 800), n_clusters=CLUSTER_NUM)
ete.model.summary()
optimizer = 'adam'
...
ete.fit(train_set, y=train_set, tol=1e-3, maxiter=2e5, save_dir='./',
    cae_weights=None,validation_data=vali_set)
